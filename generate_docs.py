...
def section_architecture():
    ...
    file_rows = [
        ...
        ["icons/",      "PNG-иконки для элементов интерфейса."],
...
def section_stock():
    ...
    col_rows = [
        ...
        ["Размер",         "Размер варианта (для размерного ряда). «Без размера» для одного варианта."],
...
def section_nomenclature():
    ...
    rows = [
        ...
        ["Вариант",      "Конкретный размер изделия: название размера (44-170, Без размера и т.д.) и уникальный полный Н/Н."],
    ]
    elems.append(feature_table(rows))
